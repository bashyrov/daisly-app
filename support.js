...
(() => {
    ...
    return (vals, ctx, key) => {
      ...
      return h(
        ...
        list.map((item, i) => {
          ...
          const itemKey = item && typeof item === "object" && item.key != null ? item.key : i;
          return h(
            getReact().Fragment,
            { key: itemKey },
  ...
  var LOCAL_BABEL_URL = "./vendor/babel.min.js";
  ...
  function createExternalModules(onResolved) {
    ...
    function ensureBabel() {
      ...
      babelLoading = loadScriptFallback(LOCAL_BABEL_URL, BABEL_URL, BABEL_SRI);
      return babelLoading;
    }
  ...
  var LOCAL_REACT_URL = "./vendor/react.production.min.js";
  var LOCAL_REACT_DOM_URL = "./vendor/react-dom.production.min.js";
  ...
  function loadScriptFallback(localSrc, remoteSrc, remoteIntegrity) {
    return loadScript(localSrc, null).catch(() => loadScript(remoteSrc, remoteIntegrity));
  }
  function loadReactUmd() {
    const w = window;
    if (w.React && w.ReactDOM) return Promise.resolve();
    return loadScriptFallback(LOCAL_REACT_URL, REACT_URL, REACT_SRI).then(() => loadScriptFallback(LOCAL_REACT_DOM_URL, REACT_DOM_URL, REACT_DOM_SRI)).then(() => void 0);
  }
  function init() {
